...
const MedicalRecords = ({ onBack }: MedicalRecordsProps) => {
  ...
  const saveMutation = useMutation({
    mutationFn: (vars: { id?: string; record: Partial<MedicalRecord> }) =>
      vars.id
        ? updateMedicalRecord(vars.id, vars.record)
        : saveMedicalRecord(vars.record as Omit<MedicalRecord, 'id'>),
    onSuccess: (_data, vars) => {
      queryClient.invalidateQueries({ queryKey: ['medical-records'] });
      toast({
        title: vars.id ? "更新成功" : "保存成功",
        description: vars.id ? "医疗记录已更新" : "医疗记录已保存",
      });
  ...
  const handleSave = () => {
    ...
    saveMutation.mutate({ id: editingRecord?.id, record: formData });
  };
  ...
  if (isError) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-green-50 p-4">
        ...
        <div className="max-w-md md:max-w-2xl lg:max-w-3xl mx-auto text-center py-12 space-y-4" role="alert">
          <p className="text-gray-600">医疗记录加载失败，请检查网络后重试</p>
