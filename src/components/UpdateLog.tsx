...
const UpdateLog = ({ onBack }: UpdateLogProps) => {
  const updates = [
    ...
    },
    {
      ...
      title: "管理员通知系统全面升级",
      items: [
        "🔔 全模块活动通知：管理员现在可以收到用户使用所有模块的实时通知",
        "📊 用户活跃度监控：精确追踪各模块使用频率，深度了解用户行为",
        "🎯 智能活动描述：详细记录用户操作内容、类型和时间",
        "⚡ 实时通知推送：用户操作后毫秒级通知管理员",
        "🔧 统一通知架构：标准化的通知服务，支持扩展和维护",
        "📱 覆盖健康模块：医疗记录、糖尿病记录、美尼尔记录等全部监控",
        "🎮 功能模块监控：游戏活动、英语学习、数据导出等全面追踪",
        "🛍️ 商业行为分析：积分商店购买行为实时监控和统计",
        "🎵 语音活动追踪：语音记录创建、删除和语音学习使用情况",
        "📈 管理后台优化：通知列表显示、已读标记和批量操作功能"
      ]
    },
    {
      version: "v2.5.6",
      ...
      title: "解压小游戏大幅扩充",
      items: [
        "🐍 新增经典贪吃蛇游戏，支持WASD和方向键控制",
        "🎯 新增2048数字合并游戏，支持触摸和键盘操作",
        "🫧 新增泡泡消消乐游戏，快节奏点击体验",
        "⚡ 游戏列表扩展至7款，提供更丰富的解压选择",
        "🎮 所有新游戏支持键盘和鼠标双重操作模式",
        "🏆 完善的计分系统和游戏结束提示",
        "🔄 智能游戏重置和暂停功能",
        "📱 优化移动端游戏体验，响应式布局",
